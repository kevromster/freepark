...
public:
	FontForMaxVal() {
		cvInitFont(&m_font, CV_FONT_HERSHEY_PLAIN, 1.f, 1.f);
	}

	const CvFont & getFont() const {return m_font;}
...
struct MaxLocation {
	MaxLocation(unsigned int _uScaleIdx) : m_uScaleIdx(_uScaleIdx), m_dValue(0.) {m_point.x = m_point.y = 0;}
...
static
void _drawBBox(IplImage * _pHoughSpace, IplImage * _pOrigImage, unsigned int _uScaleIdx) {
	assert(_pHoughSpace != NULL && _pOrigImage != NULL);
	CvPoint maxloc = {0,0};
	static const FontForMaxVal font;

	const double dThreshold = 80.;
	double dMaxVal = 0;

	for (;;) {
		cvMinMaxLoc(_pHoughSpace, NULL, &dMaxVal, NULL, &maxloc);

		if (dMaxVal < dThreshold)
			break;

		cout << "!! found dMaxVal " << dMaxVal << endl;
		ostringstream s;
		s << dMaxVal;

		const int origW = 100;
		const int origH = 40;
		const CvSize sz = cvSize(origW, origH);
		const CvSize szScaled = cvSize(int(origW/scales[_uScaleIdx]+0.5), int(origH/scales[_uScaleIdx]+0.5));
		const CvRect bbox = cvRect(maxloc.x - sz.width/2, maxloc.y - sz.height/2, sz.width, sz.height);
		const CvRect bboxScaled = cvRect(int(maxloc.x/scales[_uScaleIdx]+0.5) - szScaled.width/2, int(maxloc.y/scales[_uScaleIdx]+0.5) - szScaled.height/2, szScaled.width, szScaled.height);

		cvRectangleR(_pOrigImage, bboxScaled, CV_RGB(255, 0, 0));
		cvPutText(_pOrigImage, s.str().c_str(), cvPoint(bboxScaled.x, bboxScaled.y-2), &font.getFont(), CV_RGB(255, 0, 0));

		// TODO: maybe use mask instead of filling found maximus by black color
		cvRectangleR(_pHoughSpace, bbox, CV_RGB(0, 0, 0), CV_FILLED);
	}
}

static
void _drawBBoxes(const vector<vector<IplImage*> > & _scaleHoughImages, IplImage * _pOrigImage) {
	const double dThreshold = 100.;
	static const FontForMaxVal font;
	cout << "Searching for max locations..." << endl;

	// original size for which our HoughForest is trained
	const CvSize origSz = cvSize(100, 40);

	for (int cnt = 0;; ++cnt) {
		MaxLocation maxLoc(0);

		// find max location through all scaled hough images
		for(unsigned int uScaleIdx = 0; uScaleIdx < _scaleHoughImages.size(); ++uScaleIdx) {
			IplImage* tmp = cvCreateImage(cvSize(_scaleHoughImages[uScaleIdx][0]->width, _scaleHoughImages[uScaleIdx][0]->height) , IPL_DEPTH_8U , 1);

			// do not work now with different ratios
			assert(_scaleHoughImages[uScaleIdx].size() == 1);
			cvConvertScale(_scaleHoughImages[uScaleIdx][0], tmp, out_scale);

			//ostringstream stmp;
			//stmp << outpath << "/hough-for-search-max-loc-" << cnt << "-scale-" << uScaleIdx << ".png";
			//cvSaveImage(stmp.str().c_str(), tmp);

			MaxLocation curMaxLoc(uScaleIdx);
			cvMinMaxLoc(tmp, NULL, &curMaxLoc.m_dValue, NULL, &curMaxLoc.m_point);
		...
		cout << "found max value:" << maxLoc.m_dValue << endl;
		if (maxLoc.m_dValue < dThreshold)
			break;

		// draw bbox around found max location
		const CvSize szScaled = cvSize(int(origSz.width/scales[maxLoc.m_uScaleIdx]+0.5), int(origSz.height/scales[maxLoc.m_uScaleIdx]+0.5));
		const CvRect bboxScaled = cvRect(int(maxLoc.m_point.x/scales[maxLoc.m_uScaleIdx] - szScaled.width/2. + 0.5), int(maxLoc.m_point.y/scales[maxLoc.m_uScaleIdx] - szScaled.height/2. + 0.5), szScaled.width, szScaled.height);

		ostringstream s;
		s << maxLoc.m_dValue;
		cvRectangleR(_pOrigImage, bboxScaled, CV_RGB(255, 0, 0));
		cvPutText(_pOrigImage, s.str().c_str(), cvPoint(bboxScaled.x+2, bboxScaled.y+12), &font.getFont(), CV_RGB(255, 0, 0));

		// zeroize bbox on all hough scaled images and search max location again
		for(unsigned int uScaleIdx = 0; uScaleIdx < _scaleHoughImages.size(); ++uScaleIdx) {
			assert(_scaleHoughImages[uScaleIdx].size() == 1);

			const double dScaleFactor = double(_scaleHoughImages[uScaleIdx][0]->width) / double(_scaleHoughImages[maxLoc.m_uScaleIdx][0]->width);
			const CvSize sz = cvSize(int(origSz.width*dScaleFactor + 0.5), int(origSz.height*dScaleFactor + 0.5));
			const CvRect bbox = cvRect(int(maxLoc.m_point.x*dScaleFactor - sz.width/2. + 0.5), int(maxLoc.m_point.y*dScaleFactor - sz.height/2. + 0.5), sz.width, sz.height);

			// TODO: maybe use mask instead of filling found maximus by black color
			cvRectangleR(_scaleHoughImages[uScaleIdx][0], bbox, CV_RGB(0, 0, 0), CV_FILLED);
		}
	}
}
...
void detect(CRForestDetector& crDetect) {
	...
	for(unsigned int i=0; i<vFilenames.size(); ++i) {
		...
		const bool bDetectScaleBBoxes = true;
		...
		for(unsigned int k=0;k<vImgDetect.size(); ++k) {
			...
			for(unsigned int c=0;c<vImgDetect[k].size(); ++c) {
				...
				// draw bounding box in original image if NOT going to draw on scaled images
				if (!bDetectScaleBBoxes) {
					_drawBBox(tmp, img, k);
					sprintf_s(buffer,"%s/bbox.detect-%d_sc%d_c%d.png",outpath.c_str(),i,k,c);
		...
		if (bDetectScaleBBoxes) {
			_drawBBoxes(vImgDetect, img);
			sprintf_s(buffer,"%s/bbox.detect_scaled-%d.png",outpath.c_str(), i);
			cvSaveImage(buffer, img);
		}
