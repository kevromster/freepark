...
void CRPatch::extractPatches(IplImage *img, unsigned int n, int label, CvRect* box, std::vector<CvPoint>* vCenter) {
	...
	for(unsigned int i=0; i<n; ++i) {
		...
		for(unsigned int c=0; c<vImg.size(); ++c) {
			...
			cvShowImage(buffer, vLPatches[label].back().vPatch[c]);
			while (cvWaitKey() != 27);
...
void CRPatch::extractFeatureChannels(IplImage *img, std::vector<IplImage*>& vImg) {
	_extractFeatureChannelsForCars(img, vImg);
	return;
...
void CRPatch::_extractFeatureChannelsForCars(IplImage * _pImg, std::vector<IplImage*> & _vImg) {
	// 3 feature channels: L, |I_x|, |I_y|

	_vImg.resize(3);
	for(unsigned int c=0; c<_vImg.size(); ++c)
		_vImg[c] = cvCreateImage(cvSize(_pImg->width,_pImg->height), IPL_DEPTH_8U , 1);

	// Get intensity
	cvCvtColor(_pImg, _vImg[0], CV_BGR2GRAY);

	// Temporary images for computing I_x, I_y (Avoid overflow for cvSobel)
	IplImage* I_x = cvCreateImage(cvSize(_pImg->width,_pImg->height), IPL_DEPTH_16S, 1);
	IplImage* I_y = cvCreateImage(cvSize(_pImg->width,_pImg->height), IPL_DEPTH_16S, 1);

	// |I_x|, |I_y|
	cvSobel(_vImg[0],I_x,1,0,3);
	cvSobel(_vImg[0],I_y,0,1,3);

	cvConvertScaleAbs( I_x, _vImg[1], 0.25);
	cvConvertScaleAbs( I_y, _vImg[2], 0.25);

#if 0
	// for debugging only
	char buffer[40];
	for(unsigned int i = 0; i < _vImg.size();++i) {
		sprintf(buffer,"%d",i);
		cvNamedWindow(buffer,1);
		cvShowImage(buffer, _vImg[i]);
		//cvSaveImage( buffer, vImg[i] );
